...
function removeOldSeo(html) {
  return String(html || "")
    ...
    .replace(/<style\s+id=["']listing-app-boot-style["'][\s\S]*?<\/style>/gi, "")
    .replace(/<noscript\s+id=["']listing-app-noscript["'][\s\S]*?<\/noscript>/gi, "");
}
...
function injectIntoRoot(html, content) {
  ...
  if (root.test(html)) {
    return html.replace(root, (_match, attrs) => `<div${attrs}>${content}</div>`);
  }
...
function buildBootStyles() {
  return `
<style id="listing-app-boot-style">
  @keyframes listingAppBootSpin {
  ...
  #listing-app-loading::before {
    content: "";
    width: 42px;
    height: 42px;
    border: 4px solid #d6e4dc;
    border-top-color: #1a4a2e;
    border-radius: 50%;
    animation: listingAppBootSpin 0.85s linear infinite;
  }

  #listing-app-loading strong {
    font-size: 15px;
    font-weight: 800;
  }
...
<noscript id="listing-app-noscript">
  <style>
    #listing-app-loading { display: none !important; }
    #seo-listing-content { display: block !important; }
  </style>
</noscript>`;
}
...
<meta property="og:image:type" content="${escapeHtml(imageMimeType(shareImage))}">
...
function buildBody(listing, listingImage = "") {
  ...
  const rows = [
    ...
    listing?.floor !== undefined &&
    String(listing.floor).trim()
      ? ["الطابق", String(listing.floor)]
      : null,
    listing?.ownership
      ? ["الملكية", cleanText(listing.ownership, 120)]
      : null,
  ].filter(Boolean);
  ...
  const details = rows
    .map(
      ([label, value]) =>
        `<div style="display:grid;grid-template-columns:minmax(90px,130px) 1fr;gap:12px;padding:9px 0;border-bottom:1px solid #e5e7eb"><dt style="font-weight:800;color:#1A4A2E">${escapeHtml(label)}</dt><dd style="margin:0;color:#1f2937">${escapeHtml(value)}</dd></div>`
    )
    .join("");

  return `
<div id="listing-app-loading" role="status" aria-live="polite">
  <strong>جارٍ تحميل الإعلان...</strong>
</div>
<section id="seo-listing-content" aria-label="تفاصيل الإعلان العقاري" style="max-width:920px;margin:24px auto;padding:20px;font-family:Tajawal,Arial,sans-serif;line-height:1.9;direction:rtl;color:#111827;background:#fff">
  <article>
    <h1 style="margin:0 0 16px;font-size:clamp(24px,4vw,38px);line-height:1.4;color:#153d28">${escapeHtml(title)}</h1>
    ${listingImage ? `<figure style="margin:0 0 20px"><img src="${escapeHtml(listingImage)}" alt="${escapeHtml(title)}" loading="eager" style="display:block;width:100%;max-height:560px;object-fit:cover;border-radius:14px"></figure>` : ""}
    ${details ? `<dl style="margin:0 0 22px">${details}</dl>` : ""}
    <section>
      <h2 style="margin:0 0 8px;font-size:21px;color:#153d28">وصف الإعلان</h2>
      <p style="margin:0;white-space:pre-line;color:#374151">${escapeHtml(description)}</p>
    </section>
  </article>
</section>`;
}
...
function buildUnavailable(id, temporary = false) {
  ...
  return {
    ...
    body: `<section id="seo-listing-content" style="display:block!important;max-width:760px;margin:40px auto;padding:24px;font-family:Tajawal,Arial,sans-serif;line-height:1.9;direction:rtl;text-align:center"><h1 style="color:#153d28">${escapeHtml(title)}</h1><p>${escapeHtml(description)}</p><p><a href="${SITE_URL}/">العودة إلى الصفحة الرئيسية</a></p></section>`,
  };
}
  ...
  try {
    ...
    if (!listing) {
      ...
      const html = injectIntoRoot(
        template.replace("</head>", `${page.head}</head>`),
        page.body
      );
    ...
    const listingImage =
      (await fetchListingImage(id)) || getListingImageFromRow(listing);
